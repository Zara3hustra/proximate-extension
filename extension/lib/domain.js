...
export function normalizeDomain(input) {
  ...
  if (!s) throw new ValidationError('пустая строка после нормализации');
  ...
  try {
    ...
    throw new ValidationError(`неверный хост: ${input}`);
  }
  ...
  if (s.startsWith('[')) {
    throw new ValidationError('IPv6 не поддерживается');
  }
...
export function parseEntry(input) {
  ...
  if (!validateNormalized(value)) {
    throw new ValidationError(`неверное имя домена: ${input}`);
  }
  return { value, mode };
}
